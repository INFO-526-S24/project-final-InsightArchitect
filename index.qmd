...
#head(unique_countries_data, 100)
...
ggplot(data = world_data) +
  ...
  labs(title = "Headcount Ratio at International Poverty Line") +
  theme_minimal() +
  theme(legend.position = "bottom")
...
image_read("Merged_document.gif") %>%
  ...
  image_animate(fps = 10)  # Speed up the animation to 10 frames per second
```
...
pip_final_2018 <- pip_final %>%
  filter(ppp_version == 2017, year == 2010) %>%
  select(Country.Code, Country, year, Latitude, Longitude, headcount_ratio_international_povline)

# Remove duplicate country rows
pip_final_unique <- pip_final_2018 %>%
  distinct(Country, .keep_all = TRUE)

# Scale the "headcount_ratio_international_povline" column
...
globe <- create_globe(height = "100vh") %>% 
  ...
  scale_choropleth_altitude(0.03, 0.2)  # Adjust the altitude scale to match the range of your data
