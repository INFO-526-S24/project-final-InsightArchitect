...
# Define UI
ui <- fluidPage(
  titlePanel("Headcount Ratio at International Poverty Line"),
  
  # Dropdown menu for selecting year
  selectInput(
    "year",
    "Select Year:",
    choices = c(1996, 2002, 2005, 2008, 2012, 2014, 2015, 2016, 2017, 2018, 2019),
    selected = 1996
  ),
...
server <- function(input, output) {
  ...
  filtered_data <- reactive({
    pip_final %>%
      filter(year == input$year, ppp_version == 2017) %>%
      group_by(Country) %>%
      slice(1) %>%
      ungroup()
  })
  
  # Reactive expression for preparing the world map
  world_data <- reactive({
